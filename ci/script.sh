...
main() {
    ...
    case $TARGET in
        thumbv6m-none-eabi|thumbv7m-none-eabi)
            ...
            # linking with GNU GCC
            env RUSTFLAGS="-C linker=arm-none-eabi-gcc -C link-arg=-Tlink.x" cargo run --target "$TARGET" --example qemu | grep "x = 42"
            env RUSTFLAGS="-C linker=arm-none-eabi-gcc -C link-arg=-Tlink.x" cargo run --target "$TARGET" --example qemu --release | grep "x = 42"
